...
JSON_COLUMNS = (
    ("research_entities", "metadata_json"),
    ("research_revisions", "metadata_json"),
    ("research_evidence", "citation_json"),
    ("research_import_plans", "entries_json"),
...
def database_integrity_report(connection: sqlite3.Connection) -> dict:
    ...
    if _table_exists(connection, 'research_evidence'):
        for body, checksum in connection.execute('SELECT body,sha256 FROM research_evidence'):
            evidence_mismatches += hashlib.sha256(body.encode('utf-8')).hexdigest() != checksum
    if _table_exists(connection,'research_sources'):
        for payload,checksum in connection.execute('SELECT payload,source_id FROM research_sources'):
            evidence_mismatches += hashlib.sha256(payload).hexdigest()!=checksum
    ...
    return {
        ...
        "evidence_checksum_mismatch_count": evidence_mismatches,
        "valid": (
            quick_check == "ok"
