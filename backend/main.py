...
from storage import PROCESSED_DATA_DIR, RAW_DATA_DIR, REFERENCE_DATA_DIR
...
WORKSPACE_INTERFACE_PATH = Path(__file__).parent / "static" / "workspace.html"
WORKSPACE_STYLES_PATH = Path(__file__).parent / "static" / "workspace.css"
WORKSPACE_SCRIPT_PATH = Path(__file__).parent / "static" / "workspace.js"
PLOT_PREFERENCES_SCRIPT_PATH = Path(__file__).parent / "static" / "plot_preferences.js"
TRANSPORT_PLOT_SCRIPT_PATH = Path(__file__).parent / "static" / "transport_plot.js"
PHOTODETECTOR_SCRIPT_PATH = Path(__file__).parent / "static" / "photodetector.js"
RESEARCH_SCRIPT_PATH = Path(__file__).parent / "static" / "research.js"
...
def normalize_data_category(value: str | None) -> str:
    normalized = (value or "raw_measurement").strip().casefold()
    allowed = {
        "raw_measurement",
        "image",
        "reference",
    ...
    if normalized not in allowed:
        raise validation_error(
            ...
            "Data category must be raw_measurement, image, reference, derived, document, or other.",
        )
    return normalized
...
def workspace_styles():
    return FileResponse(WORKSPACE_STYLES_PATH, media_type="text/css")


@app.get("/assets/workspace.js", response_class=FileResponse)
def workspace_script():
    return FileResponse(WORKSPACE_SCRIPT_PATH, media_type="text/javascript")


@app.get("/assets/plot_preferences.js", response_class=FileResponse)
def plot_preferences_script():
    return FileResponse(PLOT_PREFERENCES_SCRIPT_PATH, media_type="text/javascript")


@app.get("/assets/transport_plot.js", response_class=FileResponse)
def transport_plot_script():
    return FileResponse(TRANSPORT_PLOT_SCRIPT_PATH, media_type="text/javascript")


@app.get("/assets/photodetector.js", response_class=FileResponse)
def photodetector_script():
    return FileResponse(PHOTODETECTOR_SCRIPT_PATH, media_type="text/javascript")


@app.get("/assets/research.js", response_class=FileResponse)
def research_script():
    return FileResponse(RESEARCH_SCRIPT_PATH, media_type="text/javascript")


@app.get("/app", response_class=FileResponse)
@app.get("/app/{workspace_path:path}", response_class=FileResponse)
def workspace_interface(workspace_path: str = ""):
    return FileResponse(
...
def imported_file_metadata_suggestions(file_id: str):
    ...
    suggestions = preview.extract_metadata_suggestions(
        ...
        content,
        imported_file.get("relative_path"),
    )
    bounds = None
...
@app.get("/files/{file_id}/transport-analysis")
def get_transport_analysis(file_id: str):
    imported_file = database.get_imported_file(file_id)
    if imported_file is None:
        raise validation_error(
            "file_not_found",
    ...
    processing_run = next(
        (
            ...
            if run["model_name"] == transport.MODEL_NAME
        ),
        None,
    )
    if processing_run is None:
        raise validation_error(
            "transport_analysis_not_found",
            "This dataset does not have a saved transport analysis.",
            404,
        )
    try:
        return transport.load_transport_result(
            processing_run,
            PROCESSED_DATA_DIR,
        )
...
@app.post("/files/{file_id}/transport-analysis")
def process_transport_analysis(file_id: str):
    imported_file = database.get_imported_file(file_id)
    if imported_file is None:
        raise validation_error(
            "file_not_found",
    ...
    try:
        return transport.process_imported_transport_file(
            imported_file,
            RAW_DATA_DIR,
            PROCESSED_DATA_DIR,
        )
...
def retrieve_imported_content(file_id: str):
    ...
    with stored_path.open('rb') as source:
        detected_type = research_import.media_type(stored_path, source.read(64))
    if detected_type in {'image/png', 'image/jpeg', 'image/tiff', 'application/pdf'}:
...
def edit_imported_file_metadata(
    ...
    for field_name in requested_fields:
        ...
        elif field_name == "data_category":
            updates[field_name] = normalize_data_category(value)
    ...
    updated = persist_metadata_update(file_id, {
        key: value for key, value in updates.items() if key != "project"
    }) if any(key != "project" for key in updates) else database.get_imported_file(file_id)
    sample_id = updated["sample_id"]
    sample_updates = {
        key: updates[key]
        for key in ("material_system", "substrate", "project")
        if key in updates
    }
    if sample_updates:
        database.update_sample_metadata(sample_id, sample_updates)
    return updated
...
async def import_file(
    ...
    data_category: str = Form("raw_measurement"),
):
    staging_path = None
    destination_directory = None
    destination_path = None
    ...
    try:
        ...
        normalized_data_category = normalize_data_category(data_category)
        if (
        ...
        imported_at = datetime.now(timezone.utc).isoformat()
        content_type = file.content_type
        if not normalized_measurement_date:
            with staging_path.open("rb") as source:
                staged_content = source.read()
            suggestions = preview.extract_metadata_suggestions(
                original_filename,
                staged_content,
                normalized_relative_path,
            )
            suggested_date = suggestions.get("measurement_date")
            normalized_measurement_date = normalize_measurement_date(
                suggested_date or imported_at[:10]
            )
