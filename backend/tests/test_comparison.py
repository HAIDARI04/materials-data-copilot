...
def test_comparison_page_is_available_as_a_dedicated_workspace():
    ...
    assert ".plot-controls>label {" in response.text
    assert "align-self: flex-start;" in response.text
    ...
    assert "peak.seriesIndex!==activeSeries.seriesIndex" in "".join(response.text.split())
